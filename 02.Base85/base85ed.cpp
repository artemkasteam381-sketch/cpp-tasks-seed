...
uint32_t read_block(const std::vector<uint8_t>& bytes, std::size_t pos)
{
...
void write_encoded_block(std::vector<uint8_t>& out, uint32_t value, std::size_t count)
{
    uint8_t block[5]{};
...
uint32_t decode_block(const std::vector<uint8_t>& b85str, std::size_t pos, std::size_t count)
{
    ...
    for (std::size_t i = 0; i < 5; ++i)
    {
        ...
        if (i < count)
        {
            digit = decode_table[b85str[pos + i]];
            if (digit < 0)
            {
...
void write_decoded_block(std::vector<uint8_t>& out, uint32_t value, std::size_t count)
{
    if (count >= 1)
    {
...
std::vector<uint8_t> encode(const std::vector<uint8_t>& bytes)
{
    ...
    {
        write_encoded_block(out, read_block(bytes, pos), 5);
        pos += 4;
    }
    ...
    if (tail != 0)
    {
        ...
        write_encoded_block(out, read_block(padded, 0), tail + 1);
    }
...
std::vector<uint8_t> decode(const std::vector<uint8_t>& b85str)
{
    ...
    {
        write_decoded_block(out, decode_block(b85str, pos, 5), 4);
        pos += 5;
    }
    ...
    if (tail != 0)
    {
        write_decoded_block(out, decode_block(b85str, pos, tail), tail - 1);
    }
